...
const Diary = ({data, api}) => {
    ...
    const columns: ColumnsType<DataType> = [
        ...
        },
        {
            ...
            width: 200,
            // onCell: (record: any) => record.type === 'summary'  && ({className: record.PnL > 0 ? 'profit' : 'loss'}),
            render: (_, row: any) => row.type !== 'summary' && <Select key={`${row.id}-reason-select`} size="small" style={{width: '180px'}} allowClear placeholder="Select reason..." {...selectProps(row)}/>
        },
        {
            title: 'Comment',
            dataIndex: 'comment',
            key: 'comment',
            // onCell: (record: any) => record.type === 'summary'  && ({className: record.PnL > 0 ? 'profit' : 'loss'}),
            render: (_, row: any) => row.type !== 'summary' && <Input key={`${row.id}-comment-input`} size="small" allowClear placeholder="Add comment..." {...inputProps(row)}/>
        },
    ];
    ...
    return <>
        ...
        <Table onRow={(row: any) => row.type === 'summary' && {className: row.PnL > 0 ? 'profit' : 'loss'}} rowKey="id" columns={columns} dataSource={data.positions} size="small" pagination={false}
